---
"@reponova/lang-cpp": minor
---

Add C++ language plugin: `@reponova/lang-cpp` extracts symbols, imports, and references from C++ source files (`.cpp` / `.cc` / `.cxx` / `.c++` / `.hpp` / `.hh` / `.hxx` / `.h++`) using the official `tree-sitter-cpp` v0.23.4 WASM grammar.
