# `@reponova/bootstrap-plugin`

End-to-end helper that mirrors what the Release workflow's publish matrix does for an already-bootstrapped plugin, but from a maintainer's machine. Used:

1. The very first time a new `@reponova/lang-*` is published — npm has a chicken-and-egg constraint between OIDC Trusted Publisher and the first publish that no CI workflow can solve on its own.
2. As the safe recovery tool when one job of the publish matrix fails (network flake, npm outage, etc.) — every step short-circuits when its effect is already in place, so re-running the script after a partial success is always safe.

The script performs four idempotent steps:

| # | Step | What it does |
|---|---|---|
| 1 | `npm publish --access public` | From `packages/<package-dir>`. Skipped if `<name>@<version>` is already on the registry; if the registry's CDN has not propagated yet and the publish itself returns `EPUBLISHCONFLICT`, the failure is recognised as already-published and the script moves on. |
| 2 | `pnpm trust:configure --apply` | From the monorepo root. Configures Trusted Publishers for all `@reponova/lang-*`. Idempotent on already-configured ones. |
| 3 | `git tag <name>@<version>` + `git push origin refs/tags/<tag>` | From the monorepo root. Skipped if the tag already exists locally or on `origin`. |
| 4 | `gh release create <tag> --notes-file <chunk from CHANGELOG.md>` | From the monorepo root. Notes are extracted via [`.github/scripts/extract-changelog.mjs`](../../.github/scripts/extract-changelog.mjs) — the same script the Release workflow uses. Skipped if the GH Release already exists. |

After step 4 finishes for a brand-new package, a click on **Re-run failed jobs** in the failing GitHub Actions run flips the matrix to all-green: each step detects the work as already done and exits 0.

## When to use it

### A: Brand-new plugin

You merged the PR that introduces `packages/lang-<id>/`. Changesets opened a Version PR (`0.0.0 → 0.1.0`). You merged the Version PR. The Release workflow ran and the matrix job for the new plugin failed (OIDC publish refused: no Trusted Publisher).

```bash
git checkout main && git pull
pnpm bootstrap-plugin lang-<id>
# then: GitHub Actions UI → Re-run failed jobs
```

### B: Recovery after any other isolated matrix failure

Same procedure. The script's idempotence means it can re-publish if needed, or skip the publish and only fix what's missing (tag, release, etc.).

### C: Avoid the red CI run entirely

If you'd rather have an all-green CI run on the very first publish, run bootstrap on the `changeset-release/main` branch *before* merging the Version PR:

```bash
git fetch origin
git checkout changeset-release/main
pnpm install
pnpm bootstrap-plugin lang-<id>
git checkout main
gh pr merge <version-pr-number> --squash
```

This is purely an aesthetic preference; the end state is identical to A.

## Options

| Flag | Effect |
|---|---|
| `-y`, `--yes` | Skip confirmation prompts |
| `--skip-publish` | Don't run `npm publish` |
| `--skip-trust` | Don't run `pnpm trust:configure` |
| `--skip-tag` | Don't create / push the git tag |
| `--skip-release` | Don't create the GitHub Release |
| `-h`, `--help` | Show usage |

Combine `--skip-*` flags freely when you want to redo a specific step in isolation (rare).

## Prerequisites

- npm CLI **>= 11.15.0**. Check with `npm --version`. If older: `npm install -g npm@latest`. The minimum is gated up-front by the script's preflight: 11.15 introduced `--allow-publish` and the `permissions` field that the registry now mandates for trust payloads (npm/cli#9248). With anything older, the bootstrap exits before publishing — preventing the "publish succeeded, trust failed, re-run hits `EPUBLISHCONFLICT`" partial-success state.
- Logged in to npm: `npm login`.
- GitHub CLI (`gh`) installed and authenticated: `gh auth login`.
- The package's source builds locally (the `prepublishOnly` hook should run `pnpm build`, which is the case for plugins generated by `pnpm scaffold`).

### Idempotency, in detail

Step 1 (`npm publish`) is idempotent on **two** levels:

- **Pre-flight probe** — `npm view <name>@<version>` against the public registry. If the version is already there, we skip the publish entirely.
- **Post-mortem net** — if the probe missed (the npm registry's CDN can lag a few seconds-to-minutes behind a fresh publish), the publish itself is allowed to run; if it fails with `EPUBLISHCONFLICT` / "cannot publish over the previously published versions" / `403 publish over`, the script logs `treating as already published` and continues to step 2. So a re-run after any partial success — including a brand-new package whose first publish succeeded but whose first trust call failed — converges to all-green without ever rejecting a real artefact.

Steps 2–4 are idempotent on first principles: trust-configure short-circuits when the trust is already on file (post-mortem `npm trust list`); the git tag step short-circuits if the tag exists locally **or** on `origin`; the GH Release step short-circuits if `gh release view <tag>` succeeds.

### A note on 2FA prompts during step 2 (trust)

The npm registry classifies trust operations into two tiers:

- `npm trust list <pkg>` (read) **does** honor the "skip 2FA for the next 5 minutes" CLI cooldown. The fast-path skip on already-configured packages benefits from accepting that toggle on the first publish.
- `npm trust github <pkg> --allow-publish` (write) **does not** honor the cooldown — every successful call requires a fresh webauth handshake, regardless of the toggle. So expect to tap your security key once per **newly-configured** package (zero taps for already-configured packages while the cooldown is active).

Step 2 calls into [`@reponova/trust-configure`](../trust-configure/README.md), which routes the write path through a [`node-pty`](https://github.com/microsoft/node-pty) pseudo-terminal so the webauth prompt works correctly on Windows (without a PTY, `child_process.spawn` + `cmd.exe /c npm.cmd` crashes the npm CLI's webauth handler with `STATUS_STACK_BUFFER_OVERRUN` the moment the registry asks for an OTP).

## Manual fallback

If the script fails for some reason, the same operations done by hand are:

```bash
cd packages/lang-<id>
npm publish --access public
cd ../..
pnpm trust:configure --apply

TAG="@reponova/lang-<id>@<version>"
git tag "$TAG"
git push origin "refs/tags/$TAG"

NOTES=$(node .github/scripts/extract-changelog.mjs \
  "packages/lang-<id>/CHANGELOG.md" "<version>")
gh release create "$TAG" --title "$TAG" --notes "$NOTES"
```
